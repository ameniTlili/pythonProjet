...
class Plateau:
    ...
    def est_gagnant(self, pion):
        ...
        for i in range(0, 3):
            for j in range(0, 3):
                if ((self.cases[i, 0].contenu == pion) and (self.cases[i, 1].contenu == pion) and (
                        self.cases[i, 2].contenu == pion)):
                    return True
                elif ((self.cases[0, j].contenu == pion) and (self.cases[1, j].contenu == pion) and (
                        self.cases[2, j].contenu == pion)):
                    return True
        ...
        for i in range(0,3):
            if(self.cases[i,2-i].contenu==pion):
                d1=d1+1
        if(d1==3):
            gagne=True
    ...
    def choisir_prochaine_case(self, pion):
        ...
        numLigne = randrange(0,3)
        numColonne = randrange(0,3)
        while (self.position_valide( numLigne, numColonne) == False ) :
              numLigne = randrange(0,3)
              numColonne = randrange(0,3)
